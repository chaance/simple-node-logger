...
const RollingFileAppender = function(options) {
    ...
    this.createFileName = function(now) {
        let date = now ? now instanceof Date ? now : new Date(now) : new Date(Date.now());
  
        /** @type {string} */
        let dt;

        if (typeof formatDate === 'function') {
            dt = formatDate.call(appender, date.getTime());
        } else {
            let dateString = date.toISOString();
            dt = dateString.length === 24
                ? dateString.slice(0, 10).replace(/-/g, '.') // YYYY.MM.DD
                : dateString.slice(1, 13).replace(/-/g, '.'); // YYYYY.MM.DD
        }
